...
output_fileName='merge_fimo_result.xml'

# iterate
cat header_file.xml >  ${output_fileName}
...
for file_name in $(ls fimo_output1|grep fimo_); do
	...
	sed -n '2,$p' fimo_output1/${file_name}/fimo.xml >>  ${output_fileName}
done


for file_name in $(ls fimo_output2|grep fimo_); do
        echo ${file_name} ;
	sed -n '2,$p' fimo_output2/${file_name}/fimo.xml >>  ${output_fileName}
done

...
- directed-message passing neural network
  To predict molecular properties directly from the graph structure of the molecule. A node is atom. An edge is bond.
  ...
  - `paper` A Deep Learning Approach to Antibiotic Discovery | Cell, 2020
    Methods:
    - Reconstruction of the molecular graph with SMILES
    - Initialize a feature vectors
      - atom features
      - bond features
    - train
    - additional molecular-level features from RDKit
    - Bayessian hyperparameter optimization
    - ensembling
      - training non-ensembled model
        - 80% training set, 10% validation set, 10% test set (20 times random split)
        - 30 epochs
      - training ensemble of optimized model
        - 90% training set, 10% validation set (20 times random split)
      - prediction on Drug Repurposing Hub
      - retraining on ZINC15
      - final prediction 
